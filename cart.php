...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
    ...
    if ($type === 'product') {
        ...
        $stock = getBoxStock($connect, $id);
    }

    $stock = (int)$stock;

    if ($stock <= 0) { 
    $_SESSION['error_msg'] = "Product out of stock";
    ...
    $_SESSION['error_msg'] = "Quantity exceeds available stock";
    ...
    $_SESSION['error_msg'] = "Not enough stock available";
...
<div class="form-bg"> 
    <div class="order-summary-box mb-2"> 
        ...
        <?php $_SESSION['cart'] = $_SESSION['cart'] ?? [];
    ...
    <div class="order-card"> 
        ...
        <form action="process_order.php" method="POST" id="orderForm"> 
            ...
            <div class="row"> 
                <div class="col-md-6"> 
                    <input type="text" name="fname" class="form-control" placeholder="First Name" value="<?= htmlspecialchars($_SESSION['user_fname'] ?? '') ?>" required> 
                </div> 

                <div class="col-md-6"> 
                    <input type="text" name="lname" class="form-control" placeholder="Last Name" value="<?= htmlspecialchars($_SESSION['user_lname'] ?? '') ?>" required> 
                </div> 
            </div> 

            <input type="tel" name="phone" class="form-control mt-2" placeholder="Phone Number" value="<?= htmlspecialchars($_SESSION['user_phone'] ?? '') ?>" required> 

            <div class="mt-3"> 
                <label class="form-label d-block">How do you want to receive your order?</label> 

                <div class="form-check-inline"> 
                    <input class="form-check-input" type="radio" name="order_state" value="pickup" checked> 
                ...
                <div class="d-flex justify-content-center mt-5 mb-3"> 
                    ...
                    <button type="submit" name="cancel_order" class="btn-submit cancel-btn w-48" style="margin-left:15px;">Cancel Order</button> 
                </div> 
            </div> 
